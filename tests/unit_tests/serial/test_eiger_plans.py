...
@pytest.mark.parametrize(
    "well_position,well_num", [({1: [1, 2, 3]}, 1), ({2: [1, 2, 3]}, 0)]
)
def test_run_eiger(
    ...
    mock_bps_kickoff.assert_called_once_with(eh2_eiger, wait=True)
    mock_bps_complete.assert_called_with(eh2_eiger, wait=True)
...
def test_run_small_plan(
    ...
    mock_setup_small_plan.assert_called_once_with(
        parameters.panda_rotation_params, eh2_eiger, mock_panda, serial_stages
    )
